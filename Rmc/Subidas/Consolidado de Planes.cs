...
{
    public partial class Consolidado_de_Planes : Telerik.WinControls.UI.RadForm
    {
        private string connectionString = Properties.Settings.Default.TracerConnectionString;
        string sqlQuery = "SELECT sku, socktype, dc, dozens as Docenas, QR FROM dbo.pmc_ConsolidadoPlanes;";
        private Timer timer;
        private DataTable excelDataTable = null;

        public Consolidado_de_Planes()
        {
            InitializeComponent();

            if (BrowseConsolidado.DialogType == Telerik.WinControls.UI.BrowseEditorDialogType.OpenFileDialog)
            {
                OpenFileDialog dialog = (OpenFileDialog)BrowseConsolidado.Dialog;
            ...
            try
            {
                using (SqlConnection conn = new SqlConnection(connectionString))
                {
                    ...
                    using (SqlCommand cmd = new SqlCommand(sqlQuery, conn))
                    {
                        ...
                        GridConsolidado.DataSource = dt;
                    }
                }
                lblNumRegistrosOrden.Text = GridConsolidado.RowCount.ToString();
            }
            catch (Exception ex)
            {
        ...
        {
            string[] columnasEsperadas = { "sku", "socktype", "dc", "Docenas", "QR" };

            if (dt.Columns.Count != 5)
            {
                RadMessageBox.Show($"Cantidad de columnas incorrecta. Esperadas: 5, Encontradas: {dt.Columns.Count}",
        ...
        private void TimerTick(object sender, EventArgs e)
        {
            ...
            try
            {
                if (string.IsNullOrEmpty(BrowseConsolidado.Text))
                {
                    timer.Stop();
                    return;
                }

                byte[] bytes = File.ReadAllBytes(BrowseConsolidado.Text);
        ...
        private void BrowseConsolidadoValueChanged(object sender, EventArgs e)
        {
            if (this.BrowseConsolidado.Value != null && !string.IsNullOrEmpty(this.BrowseConsolidado.Value.ToString()))
            {
                string filePath = this.BrowseConsolidado.Value.ToString();

                if (!File.Exists(filePath))
                {
                    RadMessageBox.Show("El archivo seleccionado no existe.", "Error de Archivo", MessageBoxButtons.OK, RadMessageIcon.Error);
                    return;
                }
                ...
                try
                {
                    ...
                    bool sheet1Exists = wb.Worksheets.Any(ws => ws.Name == "Sheet1");

                    if (!sheet1Exists && wb.Worksheets.Count > 0)
                    {
                ...
                {
                    RadMessageBox.Show($"Error al abrir el documento: {ex.Message}", "Error de Archivo", MessageBoxButtons.OK, RadMessageIcon.Error);
                }
            }
        }

        private async void ProcesarConsolidadoClick(object sender, EventArgs e)
        {
            if (this.BrowseConsolidado.Value == null || string.IsNullOrEmpty(this.BrowseConsolidado.Value.ToString()))
            {
                RadMessageBox.Show("Por favor seleccione un archivo primero.", "Advertencia", MessageBoxButtons.OK, RadMessageIcon.Exclamation);
                return;
            }
            ...
            try
            {
                ...
                BrowseConsolidado.Enabled = false;
            ...
            finally
            {
                ...
                BrowseConsolidado.Enabled = true;
            }
        }
        ...
        private bool LimpiarTabla()
        {
            try
            {
                ...
                {
                    conn.Open();
                    using (SqlCommand cmd = new SqlCommand("DELETE FROM pmc_ConsolidadoPlanes", conn))
                    {
        ...
        private int InsertarDatos(DataTable dt)
        {
            try
            {
                using (SqlConnection conn = new SqlConnection(connectionString))
                {
                    ...
                    {
                        bulkCopy.DestinationTableName = "pmc_ConsolidadoPlanes";

                        bulkCopy.ColumnMappings.Add("sku", "sku");
                        bulkCopy.ColumnMappings.Add("socktype", "socktype");
                        bulkCopy.ColumnMappings.Add("dc", "dc");
                        bulkCopy.ColumnMappings.Add("Docenas", "dozens");
                        bulkCopy.ColumnMappings.Add("QR", "QR");
        ...
        private int ContarRegistrosEnBD()
        {
            try
            {
                ...
                {
                    conn.Open();
                    using (SqlCommand cmd = new SqlCommand("SELECT COUNT(*) FROM pmc_ConsolidadoPlanes", conn))
                    {
        ...
        private void LimpiarInterfaz()
        {
            this.Invoke(new Action(() =>
            {
                ...
                BrowseConsolidado.Text = "";
                BrowseConsolidado.Value = null;
                excelDataTable = null;
            }));
        }

        private void BrowseConsolidado_Click(object sender, EventArgs e)
        {
            ...
            if (f.ShowDialog() == DialogResult.OK)
            {
                try
                {
                    BrowseConsolidado.Text = f.FileName;
                    BrowseConsolidado.Value = f.FileName;
        ...
        private DataTable LeerDatosExcel(string filePath)
        {
            try
            {
                ...
                dt.Columns.Add("socktype", typeof(string));
                dt.Columns.Add("dc", typeof(string));
                dt.Columns.Add("Docenas", typeof(decimal));
                ...
                for (int i = startRow; i < rowCount; i++)
                {
                    ...
                    for (int j = 0; j < colCount && j < dt.Columns.Count; j++)
                    {
                        ...
                        valor = System.Text.RegularExpressions.Regex.Replace(valor, @"\s+", " ").Trim();
                        ...
                        if (j == 3)
                        {
                            if (decimal.TryParse(valor, out decimal docenas))
                                dr[j] = docenas;
                            else
                                dr[j] = 0m;
                        }
                        ...
                        {
                            string qrLimpio = System.Text.RegularExpressions.Regex.Replace(valor, @"\s", "");
                            qrLimpio = qrLimpio.Length > 20 ? qrLimpio.Substring(0, 20) : qrLimpio;
                            dr[j] = qrLimpio;

                            if (i < startRow + 5)
                            {
                                Console.WriteLine($"Fila {i}, QR original: '{cellValue}'");
                                Console.WriteLine($"Fila {i}, QR limpio: '{qrLimpio}' (Longitud: {qrLimpio.Length})");
                                Console.WriteLine($"Fila {i}, Caracteres QR: {string.Join("-", qrLimpio.Select(c => ((int)c).ToString()))}");
                            }
                        }
                        else
                        {
        ...
        private void Consolidado_de_Planes_Load(object sender, EventArgs e)
        {
            lblNumRegistrosOrden.Text = GridConsolidado.RowCount.ToString();
        }
    }
