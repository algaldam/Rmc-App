...
{
    public partial class BagIrr : Telerik.WinControls.UI.RadForm
    {
        private string connectionString = Properties.Settings.Default.TracerConnectionString;
        string sqlQuery = "SELECT [pmc_SACA_IRR] Saca_IRR,[pmc_item] as Item,[pmc_descripcion] as Descripcion,[pmc_factor100] as 'Factor100%' FROM [pmc_Bag_IRR]";
        private Timer timer;
            ...
            try
            {
                using (SqlConnection conn = new SqlConnection(connectionString))
                {
                    ...
                    using (SqlCommand cmd = new SqlCommand(sqlQuery, conn))
                    {
                        ...
                        {
                            GridBag.DataSource = dt;
                            lblNumRegistrosOrden.Text = GridBag.RowCount.ToString();
                        }
                    }
                }
            }
        ...
        {
            string[] columnasEsperadas = { "Saca_IRR", "Item", "Descripcion", "Factor100%" };

            if (dt.Columns.Count != 4)
            {
                RadMessageBox.Show($"Cantidad de columnas incorrecta. Esperadas: 4, Encontradas: {dt.Columns.Count}",
        ...
        private void TimerTick(object sender, EventArgs e)
        {
            ...
            try
            {
                if (string.IsNullOrEmpty(BrowseBag.Text))
                {
                    timer.Stop();
                    return;
                }

                byte[] bytes = File.ReadAllBytes(BrowseBag.Text);
        ...
        private async void ProcesarBag_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                ProcesarBag.Enabled = false;
                BrowseBag.Enabled = false;
            ...
            finally
            {
                ...
                radWaitingBar1.Visible = false;
                ProcesarBag.Enabled = true;
                BrowseBag.Enabled = true;
            }
        }
        ...
        private bool LimpiarTabla()
        {
            try
            {
                ...
                {
                    conn.Open();
                    using (SqlCommand cmd = new SqlCommand("DELETE FROM pmc_Bag_IRR", conn))
                    {
        ...
        private int InsertarDatos(DataTable dt)
        {
            try
            {
                using (SqlConnection conn = new SqlConnection(connectionString))
                {
                    ...
                    {
                        bulkCopy.DestinationTableName = "pmc_Bag_IRR";

                        bulkCopy.ColumnMappings.Add("Saca_IRR", "pmc_SACA_IRR");
                        bulkCopy.ColumnMappings.Add("Item", "pmc_item");
                        bulkCopy.ColumnMappings.Add("Descripcion", "pmc_descripcion");
                        bulkCopy.ColumnMappings.Add("Factor100%", "pmc_factor100");
        ...
        private int ContarRegistrosEnBD()
        {
            try
            {
                ...
                {
                    conn.Open();
                    using (SqlCommand cmd = new SqlCommand("SELECT COUNT(*) FROM pmc_Bag_IRR", conn))
                    {
        ...
        private void BrowseBag_Click(object sender, EventArgs e)
        {
            ...
            if (f.ShowDialog() == DialogResult.OK)
            {
                try
                {
                    BrowseBag.Text = f.FileName;
                    BrowseBag.Value = f.FileName;
        ...
        private DataTable LeerDatosExcel(string filePath)
        {
            try
            {
                ...
                dt.Columns.Add("Factor100%", typeof(string));
