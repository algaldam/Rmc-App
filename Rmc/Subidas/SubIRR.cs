...
{
    public partial class SubIRR : Telerik.WinControls.UI.RadForm
    {
        private string connectionString = Properties.Settings.Default.TracerConnectionString;
        string sqlQuery = "SELECT [pmc_SACA_1ra] as SACA_PRIMERA,[pmc_SACA_2da] SACA_IRR,[pmc_SACA_3ra] SACA_TERCERA FROM [pmc_SACA_1as_2das_3ras]";
        private Timer timer;
            ...
            try
            {
                using (SqlConnection conn = new SqlConnection(connectionString))
                {
                    ...
                    using (SqlCommand cmd = new SqlCommand(sqlQuery, conn))
                    {
                        ...
                        {
                            GridSACA.DataSource = dt;
                            lblNumRegistrosOrden.Text = GridSACA.RowCount.ToString();
                        }
                    }
                }
            }
        ...
        {
            string[] columnasEsperadas = { "SACA_PRIMERA", "SACA_IRR", "SACA_TERCERA" };
        ...
        private void TimerTick(object sender, EventArgs e)
        {
            ...
            try
            {
                if (string.IsNullOrEmpty(BrowseSACA.Text))
                {
                    timer.Stop();
                    return;
                }

                byte[] bytes = File.ReadAllBytes(BrowseSACA.Text);
        ...
        private void BrowseSACA_ValueChanged(object sender, EventArgs e)
        {
        }

        private void BrowseSACA_Click(object sender, EventArgs e)
        {
            ...
            if (f.ShowDialog() == DialogResult.OK)
            {
                try
                {
                    BrowseSACA.Text = f.FileName;
                    BrowseSACA.Value = f.FileName;
        ...
        private async void ProcesarSACA_Click(object sender, EventArgs e)
        {
            if (string.IsNullOrEmpty(BrowseSACA.Text))
            {
            ...
            try
            {
                ...
                ProcesarSACA.Enabled = false;
                BrowseSACA.Enabled = false;
            ...
            finally
            {
                ...
                radWaitingBar1.Visible = false;
                ProcesarSACA.Enabled = true;
                BrowseSACA.Enabled = true;
            }
        }
        ...
        private bool LimpiarTabla()
        {
            try
            {
                ...
                {
                    conn.Open();
                    using (SqlCommand cmd = new SqlCommand("DELETE FROM pmc_SACA_1as_2das_3ras", conn))
                    {
        ...
        private int InsertarDatos(DataTable dt)
        {
            try
            {
                using (SqlConnection conn = new SqlConnection(connectionString))
                {
                    ...
                    {
                        bulkCopy.DestinationTableName = "pmc_SACA_1as_2das_3ras";

                        bulkCopy.ColumnMappings.Add("SACA_PRIMERA", "pmc_SACA_1ra");
                        bulkCopy.ColumnMappings.Add("SACA_IRR", "pmc_SACA_2da");
                        bulkCopy.ColumnMappings.Add("SACA_TERCERA", "pmc_SACA_3ra");
        ...
        private int ContarRegistrosEnBD()
        {
            try
            {
                ...
                {
                    conn.Open();
                    using (SqlCommand cmd = new SqlCommand("SELECT COUNT(*) FROM pmc_SACA_1as_2das_3ras", conn))
                    {
        ...
        private DataTable LeerDatosExcel(string filePath)
        {
            try
            {
                ...
                dt.Columns.Add("SACA_PRIMERA", typeof(string));
                dt.Columns.Add("SACA_IRR", typeof(string));
                dt.Columns.Add("SACA_TERCERA", typeof(string));
